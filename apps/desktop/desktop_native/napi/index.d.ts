...
export declare namespace autofill {
  export class IpcServer {
    ...
    /** Stop the IPC server. */
    stop(): void
    completeRegistration(clientId: number, sequenceNumber: number, response: autofill.PasskeyRegistrationResponse): number
    completeAssertion(clientId: number, sequenceNumber: number, response: autofill.PasskeyAssertionResponse): number
    completeError(clientId: number, sequenceNumber: number, error: string): number
  }
  export interface PasskeyAssertionRequest {
    rpId: string
    clientDataHash: Array<number>
    userVerification: autofill.UserVerification
    allowedCredentials: Array<Array<number>>
    windowXy: autofill.Position
  }
...
export declare namespace biometrics_v2 {
  ...
  export function authenticate(biometricLockSystem: biometrics_v2.BiometricLockSystem, hwnd: Buffer, message: string): Promise<boolean>
  export function authenticateAvailable(biometricLockSystem: biometrics_v2.BiometricLockSystem): Promise<boolean>
  export function enrollPersistent(biometricLockSystem: biometrics_v2.BiometricLockSystem, userId: string, key: Buffer): Promise<void>
  export function hasPersistent(biometricLockSystem: biometrics_v2.BiometricLockSystem, userId: string): Promise<boolean>
  export function initBiometricSystem(): biometrics_v2.BiometricLockSystem
  export function provideKey(biometricLockSystem: biometrics_v2.BiometricLockSystem, userId: string, key: Buffer): Promise<void>
  export function unenroll(biometricLockSystem: biometrics_v2.BiometricLockSystem, userId: string): Promise<void>
  export function unlock(biometricLockSystem: biometrics_v2.BiometricLockSystem, userId: string, hwnd: Buffer): Promise<Buffer>
  export function unlockAvailable(biometricLockSystem: biometrics_v2.BiometricLockSystem, userId: string): Promise<boolean>
}

export declare namespace chromium_importer {
  export function getAvailableProfiles(browser: string): Array<chromium_importer.ProfileInfo>
  /** Returns OS aware metadata describing supported Chromium based importers as a JSON string. */
  export function getMetadata(): Record<string, chromium_importer.NativeImporterMetadata>
  export function importLogins(browser: string, profileId: string): Promise<Array<chromium_importer.LoginImportResult>>
  ...
  export interface LoginImportResult {
    login?: chromium_importer.Login
    failure?: chromium_importer.LoginImportFailure
  }
...
export declare namespace logging {
  export function initNapiLog(jsLogFn: ((err: Error | null, arg0: logging.LogLevel, arg1: string) => any)): void
...
export declare namespace sshagent {
  ...
  export function clearKeys(agentState: sshagent.SshAgentState): void
  export function isRunning(agentState: sshagent.SshAgentState): boolean
  export function lock(agentState: sshagent.SshAgentState): void
  export interface PrivateKey {
    privateKey: string
    name: string
    cipherId: string
  }
  export function serve(callback: ((err: Error | null, arg: sshagent.SshUiRequest) => Promise<boolean>)): Promise<sshagent.SshAgentState>
  export function setKeys(agentState: sshagent.SshAgentState, newKeys: Array<sshagent.PrivateKey>): void
  ...
  export function stop(agentState: sshagent.SshAgentState): void
}
